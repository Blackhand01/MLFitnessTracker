...
bench_df = df[df["label"] == "bench"]   # panca
squat_df = df[df["label"] == "squat"]   # squat
row_df = df[df["label"] == "row"]       # rematore
ohp_df = df[df["label"] == "ohp"]       # lento avanti 
dead_df = df[df["label"] == "dead"]     # stacco da terra
...
def count_reps(dataset, cutoff=0.4, order=10, column="acc_r"):
    ...
    fig, ax = plt.subplots()
    ax.plot(data[column], color="red")
    ax.plot(peaks[f"{column}_lowpass"], "o", color="red")
...
errore = mean_absolute_error(rep_df["reps"], rep_df["reps_pred"]).round(2)
